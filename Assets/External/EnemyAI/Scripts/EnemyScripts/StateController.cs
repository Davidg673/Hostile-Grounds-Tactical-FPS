...
namespace EnemyAI
{
	...
	public class StateController : MonoBehaviour
	{
		...
		public List<Transform> SiteWayPoints;
		[Tooltip("Current bullets on weapon mag.")]
		public int bullets;
		[Space(10)]
		...
		[Range(0, 200)] public float perceptionRadius;
		...
		[HideInInspector] public int maximumBurst = 3;              // The maximum burst size on a round.
		public float blindEngageTime = 5f;       // Time to keep targeting last seen position after target leaves sight.
		...
		private float checkPoolTimer = 1f;
		public static UnityAction<GameObject> OnRemoveFromPool;


		[SerializeField] AudioSource sfxFootstepSource;
		[SerializeField] AudioClip currentFootstepClip;
		[SerializeField] AudioClip footstepClipSand;
		[SerializeField] AudioClip footstepClipConcrete;
		...
		void Update()
		{
			...
			checkPoolTimer += Time.deltaTime;

			if (animator.GetFloat("Speed") > 0.15f && playSoundOnce)
			{
				playSoundOnce = false;
				sfxFootstepSource.Play();
			}
			else if (animator.GetFloat("Speed") < 0.15f)
			{
		...
		public bool BlockedSight()
		{
			...
			if (!checkedOnLoop)
			{
				...
				try
				{
					if (aimTarget != null) target = aimTarget.position;
				}
		...
		private void SetTargetFromPool()
		{
			if (targetPool.Count == 0 || targetPool==null)
			{
				return;
			}
			GameObject closest = targetPool[0];

			foreach (GameObject target in targetPool)
			{

				float posToNewClosest = Vector3.Distance(transform.position, target.transform.position);
				float posToClosest = Vector3.Distance(transform.position, closest.transform.position);

				if (posToNewClosest < posToClosest) closest = target;
			}

			aimTarget = closest.transform;

		}

		private void RemoveFromPool(GameObject enemy)
		{
			if (targetPool.Contains(enemy))
			{
				targetPool.Remove(enemy);
				if (GameController.activeCT.Contains(enemy)) GameController.activeCT.Remove(enemy);
				if (GameController.activeT.Contains(enemy)) GameController.activeT.Remove(enemy);
			}
			
		}

		public void voidPlayFootstep()
		{
		...
		private IEnumerator CheckGroundMaterial()
		{
			...
			if (Physics.Raycast(transform.position, Vector3.down, out hit, 10f))
			{
				...
				{
					sfxFootstepSource.clip = footstepClipSand;
					newPitch = 1.8f;
				}
				if (hit.collider.gameObject.tag == "Concrete")
				{
					sfxFootstepSource.clip = footstepClipConcrete;
					newPitch = 1.5f;
				}

				if (sfxFootstepSource.pitch != newPitch)
				{
					playSoundOnce = true;
					sfxFootstepSource.pitch = newPitch;
				}
		...
		{
			List<GameObject> entityList = default;

			if (team == GameController.Team.CT) entityList = GameController.activeT;
			else entityList = GameController.activeCT;

            foreach (GameObject entity in entityList)
            {
				if (entity == this) continue;
				
				targetPool.Add(entity);
            }
        }
