...
public class AttackAction : Action
{
	...
	private void CastShot(StateController controller)
	{
		...
		// Cast shot.
		Ray ray = new Ray(controller.enemyAnimation.gunMuzzle.position, shotDirection.normalized + imprecision + new Vector3(0f,0.05f,0));
		if (Physics.Raycast(ray, out RaycastHit hit, controller.viewRadius*10f, controller.generalStats.shotMask.value))
		{
		...
		Vector3 hitNormal = default, bool organic = false, Transform target = null)
	{
		...
		else if(target && organic)
		{
			...
			if (targetHealth==null) targetHealth=target.GetComponentInParent<HealthManager>();
			if(targetHealth)
			{
